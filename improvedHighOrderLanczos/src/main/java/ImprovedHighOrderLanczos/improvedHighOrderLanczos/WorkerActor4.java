...
public class WorkerActor4 extends UntypedActor{
...
//			System.out.println("computeEpsil "+(System.nanoTime()-time));
	...
	class computeAccumEpsil implements Runnable {
		int partWLoc ;
		public void setPartWLoc(int partWLoc) {
			this.partWLoc = partWLoc;
		}
		
		@Override
		public void run() {
			// TODO Auto-generated method stub
			epsil += TensorOp.norm2(WArray[partWLoc]);
		}
		
	}
	class ComputeAccumDelta implements Runnable{
...
//				getSelf().tell(new DeltaAndEpsil(delta, epsil), getSelf());;
		...
		if(message instanceof WMessage){
			...
			if(mes.getLoc() != loc){
				...
				seq++;
				final int location = mes.getLoc();
				executor.execute(new Runnable() {
					
					@Override
					public void run() {
						// TODO Auto-generated method stub
						epsil+= TensorOp.norm2(WArray[location]);
					}
				});
