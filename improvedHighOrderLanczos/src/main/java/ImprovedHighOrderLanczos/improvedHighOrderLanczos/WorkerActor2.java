...
//			}
		...
		if(message instanceof DeltaAndEpsil){
			if(deSeq==0)
				System.out.println("[worker "+loc+" ]: "+"receive first DeltaEpsil "+(System.nanoTime()-receiveDETime));
			if(deSeq!=0)
				System.out.println("[worker "+loc+" ]: "+"receive  DeltaEpsil "+(System.nanoTime()-receiveDETime));
			...
			if(workerNum==1||deSeq == workerNum-1){
				
				iter++;
				if(iter == 101){
					manager.tell(new Stop(), getSelf());
					return;
				}
				deSeq =0;
